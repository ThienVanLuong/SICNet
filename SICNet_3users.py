'''SICNet with single SNR training'''
...
loss = tf.reduce_mean(1*tf.pow(p1 - s1, 2) + 1*tf.pow(p2 - s2, 2) + tf.pow(p3 - s3, 2))
...
def generate_data_test(num_bits, noise_std, h):
    bits1 = np.random.binomial(n=1, p=0.5, size=num_bits)
    bits2 = np.random.binomial(n=1, p=0.5, size=num_bits)
    bits3 = np.random.binomial(n=1, p=0.5, size=num_bits)
...
with tf.Session() as sess:
    ...
    y_, s1_, s2_, s3_, _ = generate_data(num_bits_train, noise_std_train, h_train)
    ...
    for ii in range(len(EbNodB)):
        ...
        # h_test = 0.6
        y_, _, _, _, bits3 = generate_data_test(num_bits_test, noise_std, h_test)
