...
for c=1:iter
    ...
    %     x = x1 + x2 + x3;
        ...
        add_position = (randsrc(1,num_sample,[0 1; (1-p_radar) p_radar]))';
        corrupted_noise = radar_power_factor*add_position.*(sqrt(noise_std3)*randn(num_sample,1));
        noise3 = noise3 + corrupted_noise;
        
        y1 = h(1)*x + noise1;
        y2 = h(2)*x + noise2;
        y3 = h(3)*x + noise3;

        h3_est = h(3)*ones(num_sample,1) + randn(num_sample,1)/sqrt(20); 
